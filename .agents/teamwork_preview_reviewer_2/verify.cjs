...
fs.writeFileSync(path.join(__dirname, 'deep_check.ts'), tsCode);

try {
  const out = execSync('npx tsx .agents/teamwork_preview_reviewer_2/deep_check.ts', { encoding: 'utf-8', cwd: path.join(__dirname, '../../') });
  console.log(out);
} catch (e) {
  console.error(e.stdout || e.stderr || e);
}
