...
export const CustomRecipeBuilderModal: React.FC<CustomRecipeBuilderModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-black/75 backdrop-blur-sm animate-fadeIn">
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="recipe-builder-dialog-title" tabIndex={-1} className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-3xl glass-panel border border-stone-200 dark:border-stone-800 shadow-2xl p-6 space-y-6">
        ...
        <div className="flex items-start justify-between pb-4 border-b border-stone-200 dark:border-stone-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 id="recipe-builder-dialog-title" className="text-xl font-extrabold text-stone-900 dark:text-stone-50">
        ...
        <div className="space-y-4 text-xs">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3.5 py-2.5 rounded-xl bg-stone-100 dark:bg-stone-800 border border-stone-200 dark:border-stone-700 font-bold text-stone-900 dark:text-stone-100"
            />
          </div>

          {/* Size Choice */}
          <div>
            <label className="block font-bold text-stone-700 dark:text-stone-300 mb-1">Boyut Seçimi</label>
            <div className="grid grid-cols-4 gap-2">
              {SIZE_OPTIONS.map(size => (
                <button
                  key={size.id}
                  onClick={() => setSizeId(size.id)}
                  className={`py-2 rounded-xl border font-bold ${sizeId === size.id ? 'bg-amber-500 text-white border-amber-500' : 'bg-stone-100 dark:bg-stone-800 text-stone-700 dark:text-stone-300'}`}
                >
                  {size.name.split(' ')[0]}
                </button>
              ))}
          ...
          {/* Milk Choice */}
          <div>
            <label className="block font-bold text-stone-700 dark:text-stone-300 mb-1">Süt Türü</label>
            <select
              value={milkId}
              onChange={(e) => setMilkId(e.target.value)}
              className="w-full px-3 py-2.5 rounded-xl bg-stone-100 dark:bg-stone-800 border border-stone-200 dark:border-stone-700 font-semibold"
            >
              {MILK_OPTIONS.map(m => (
          ...
          <div className="grid grid-cols-2 gap-2">
            <button
              ...
              className={`p-2.5 rounded-xl border flex items-center justify-between font-bold ${hasWhippedCream ? 'bg-amber-500/15 border-amber-500 text-amber-900 dark:text-amber-200' : 'bg-stone-100 dark:bg-stone-800 text-stone-700 dark:text-stone-300'}`}
            >
            ...
            <button
              ...
              className={`p-2.5 rounded-xl border flex items-center justify-between font-bold ${hasColdFoam ? 'bg-amber-500/15 border-amber-500 text-amber-900 dark:text-amber-200' : 'bg-stone-100 dark:bg-stone-800 text-stone-700 dark:text-stone-300'}`}
            >
