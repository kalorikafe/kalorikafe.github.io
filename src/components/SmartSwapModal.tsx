...
export const SmartSwapModal: React.FC<SmartSwapModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-black/70 backdrop-blur-sm animate-fadeIn">
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="smart-swap-dialog-title" tabIndex={-1} className="relative w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-3xl glass-panel border border-stone-200 dark:border-stone-800 shadow-2xl p-6 space-y-6">
        ...
        <div className="flex items-start justify-between pb-4 border-b border-stone-200 dark:border-stone-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 id="smart-swap-dialog-title" className="text-xl font-extrabold text-stone-900 dark:text-stone-50">
        ...
        <div className="space-y-3">
          {SMART_SWAPS.map((swap, idx) => (
            <div
              key={idx}
              className="p-4 rounded-2xl bg-stone-50 dark:bg-stone-800/60 border border-stone-200/80 dark:border-stone-700/80 space-y-2 hover:border-emerald-500/40 transition-all"
            >
              ...
              <div className="text-xs font-bold text-stone-800 dark:text-stone-200 flex items-center gap-1.5">
                ...
                <span className="font-normal text-stone-600 dark:text-stone-400">{swap.swapDescription}</span>
              </div>
            </div>
          ))}
        </div>
